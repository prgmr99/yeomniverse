...
const WEIGHTS = {
  adx: {
    ...
    description:
      'Average Directional Index — measures how strong a trend is, regardless of direction. ADX above 25 signals a trending market; below 20 indicates consolidation.',
  },
  maAlignment: {
    weight: 0.2,
    label: 'MA Alignment',
    description:
      'Measures how well the 8, 21, 50, and 200-period EMAs are stacked in bullish or bearish order. Perfect alignment (all EMAs in sequence) scores 100.',
  },
  rsiTrend: {
    weight: 0.15,
    label: 'RSI Trend Zone',
    description:
      'RSI in the 55–65 range during a bull trend signals momentum without exhaustion. Extremes above 80 or below 20 actually reduce the score — they signal potential reversal, not continuation.',
  },
  bbWidth: {
    weight: 0.15,
    label: 'Bollinger Band Width',
    description:
      'Measures volatility expansion from a recent squeeze. Markets that break out of a low-volatility period into expanding Bollinger Bands score highest.',
  },
  vix: {
    weight: 0.1,
    label: 'VIX (Volatility Index)',
    description:
      'In a bull context, low VIX (12–16) means calm confidence — ideal for one-way uptrends. In a bear context, high VIX (>30) confirms panic-driven one-directional selling.',
  },
  volume: {
    weight: 0.1,
    label: 'Volume Conviction',
    description:
      'Compares 20-day average volume to 50-day average. Rising volume confirms that market participants are increasingly committed to the trend direction.',
  },
  fearGreed: {
    weight: 0.05,
    label: 'Fear & Greed Calibration',
    description:
      'A small weighting that aligns sentiment with price action. Extreme Greed during a bull trend or Extreme Fear during a bear trend adds directional confirmation.',
  },
};
...
export default function OneWayIndexPage() {
  ...
  const webPageSchema = {
    ...
    description:
      'A composite indicator measuring one-directional market trend probability using 7 technical indicators.',
    url: `${DOMAIN}/one-way-index`,
    about: {
      '@type': 'Thing',
      name: 'Market Trend Analysis',
      description:
        'Technical analysis of market trend strength and directionality',
    },
  };
      ...
      <section className="pt-32 pb-16 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-xl text-white/70 max-w-2xl mb-12">
            A proprietary composite indicator that measures the probability of a
            sustained one-directional market trend — built from 7 technical
            signals across price, momentum, volatility, and sentiment.
          </p>
            ...
            {oneWay ? (
              ...
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center min-h-[200px]">
                <p className="text-white/40 text-sm">
                  Index data unavailable — check back after 8 AM KST
                </p>
              </div>
            )}
            {fearGreed ? (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-4 text-white/70 leading-relaxed">
              <p>
                Most market indicators answer the question{' '}
                <em>"which direction?"</em> — but the One-Way Market Index
                answers a different question:{' '}
                <strong className="text-white">
                  "how strongly is the market committed to that direction?"
                </strong>
              </p>
              <p>
                Financial markets spend roughly 70% of their time in ranging,
                choppy conditions where trend-following strategies consistently
                lose money. The remaining 30% — the trending periods — is where
                the majority of market returns are captured.
              </p>
              <p>
                The One-Way Index was designed to identify those trending
                windows with high confidence, by requiring multiple independent
                technical signals to simultaneously confirm directional
                conviction.
              </p>
            </div>
            <div className="space-y-4 text-white/70 leading-relaxed">
              <p>
                Unlike the Fear & Greed Index, which measures investor
                sentiment, the One-Way Index measures{' '}
                <strong className="text-white">structural trend quality</strong>{' '}
                — whether the price action itself is exhibiting the
                characteristics of a true one-directional move.
              </p>
              <p>
                A market can be in "Extreme Greed" sentiment while still being
                choppy and range-bound. Conversely, a strong downtrend can
                persist even as sentiment recovers. The One-Way Index captures
                the technical reality of what the market is actually doing,
                independent of emotional state.
              </p>
              <p>
                The index is calculated daily using S&P 500 historical price and
                volume data, combined with the CBOE Volatility Index (VIX) and
                CNN Fear & Greed data.
              </p>
            </div>
          </div>
      ...
      <section className="py-16 px-6 border-t border-white/10">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-white/60 mb-10 max-w-2xl">
            Each component is scored 0–100 and weighted by its contribution to
            trend identification. The composite score is a weighted sum.
          </p>
          <div className="space-y-6">
            {Object.entries(WEIGHTS).map(([key, info]) => {
              const componentScore =
                oneWay?.components?.[key as keyof typeof oneWay.components];
              const color =
                componentScore != null
                  ? getScoreColor(componentScore)
                  : '#86868B';
              return (
                <div
                  key={key}
            ...
            <div className="space-y-6">
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                ...
                <ul className="text-sm text-white/60 space-y-1.5">
                  <li>
                    • S&P 500 (^GSPC) — price, high, low, volume (365-day
                    history)
                  </li>
              ...
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                ...
                <p className="text-sm text-white/60">
                  Calculated once daily at 8:00 AM KST (23:00 UTC) using the
                  previous trading day's closing data. Distributed as part of
                  the FinBrief morning briefing.
                </p>
              </div>
            </div>
            <div className="space-y-6">
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                ...
                <p className="text-sm text-white/60">
                  Market direction (bull/bear) is determined by combining ADX
                  +DI/-DI differential with EMA stack alignment. When ADX
                  exceeds 25, the +DI/-DI comparison takes precedence. In weaker
                  trend conditions, EMA alignment determines direction.
                </p>
              </div>
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                <h3 className="font-semibold mb-2">Disclaimer</h3>
                <p className="text-sm text-white/60">
                  The One-Way Market Index is a technical analysis tool for
                  informational purposes only. It does not constitute financial
                  advice. Past index readings do not guarantee future market
                  performance.
                </p>
              </div>
            </div>
      ...
      <section className="py-16 px-6 border-t border-white/10">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-4">
            Get the Index Delivered Daily
          </h2>
          <p className="text-white/60 mb-8 max-w-xl mx-auto">
            The One-Way Index is published every morning alongside AI-curated
            financial news. Subscribe to FinBrief to receive it directly in
            Telegram.
          </p>
