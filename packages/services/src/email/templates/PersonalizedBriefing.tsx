        ...
        <Container style={styles.container}>
          ...
          {isPaid && watchlist && watchlist.length > 0 && (
            <Section style={styles.section}>
              <Heading as="h2" style={styles.sectionTitle}>🎯 내 관심종목</Heading>
              <Hr style={styles.hr} />
              {watchlist.map((stock, index) => (
                <Section key={index} style={styles.stockItem}>
                  <Text style={styles.stockName}>
                    {stock.name} {stock.price.toLocaleString()}원
                    <span style={stock.changePercent >= 0 ? styles.positive : styles.negative}>
                      {' '}({stock.changePercent >= 0 ? '+' : ''}{stock.changePercent.toFixed(1)}%)
                    </span>
                    {stock.changePercent >= 1 ? ' 🐂' : stock.changePercent <= -1 ? ' 🐻' : ''}
                  </Text>
                  {isPro && stock.rsi && (
                    <Text style={styles.indicators}>
                      RSI {stock.rsi.toFixed(0)} | {stock.macdSignal || 'MACD 중립'}
                    </Text>
                  )}
                  {stock.aiSummary && (
                    <Text style={styles.aiSummary}>→ AI: "{stock.aiSummary}"</Text>
                  )}
                </Section>
              ))}
            </Section>
          ...
          {/* Main News Section */}
          <Section style={styles.section}>
            <Heading as="h2" style={styles.sectionTitle}>📰 오늘의 주요 뉴스</Heading>
            <Hr style={styles.hr} />
            {topNews.map((news, index) => (
              <Section key={index} style={styles.newsItem}>
                <Text style={styles.newsTitle}>
                  {news.sentiment === 'bull' ? '🟢' : news.sentiment === 'bear' ? '🔴' : '⚪'} {news.title}
                </Text>
          ...
          <Section style={styles.footer}>
            ...
            <Text style={styles.footerText}>
              <Link href={`${dashboardUrl}/../api/unsubscribe?token=${unsubscribeToken}`} style={styles.unsubscribe}>
                구독 해지
              </Link>
              {' | '}
            ...
            <Text style={styles.copyright}>© 2026 FinBrief. All rights reserved.</Text>
          </Section>
        </Container>
      </Body>
    </Html>
